...
\end{itemize}
Constants $d_1$ in $d_2$ can be calculated using the following formulas:
\begin{equation}
    \begin{aligned}
...
In such a case, a slight VaR modification is necessary.
...
VaR for an investment therefore equals 

\begin{equation}
    VaR = MV\cdot \alpha_{cl} \cdot \sigma,
\end{equation}

where $MV$ is the asset's market value.
...
the returns of the underlying asset, we to calculate a few additional parameters in 
...
or rather that a relative change of portfolio is a linear function of returns (of stocks, bonds\doths).
